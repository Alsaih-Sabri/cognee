...
import { CTAButton, Input, NeutralButton, Select, IconButton } from "@/ui/elements";
  ...
  return (
    <>
      <div className="flex w-full">
        ...
        <span className="whitespace-nowrap text-gray-900 dark:text-white">Node Details</span>
        {/* </button> */}
      ...
      <div className="pt-4">
        ...
        <>
          <div className="w-full flex flex-row gap-2 items-center mb-4">
            <label className="text-gray-600 dark:text-gray-300 whitespace-nowrap flex-1/5">Graph Shape:</label>
          ...
          {isAddNodeFormOpen ? (
            <form className="flex flex-col gap-4" onSubmit={() => { }}>
              <div className="flex flex-row gap-4 items-center">
                <span className="text-gray-600 dark:text-gray-300 whitespace-nowrap">Source Node ID:</span>
              ...
              <div className="flex flex-col gap-4 items-end">
                {nodeProperties.map((property) => (
                  <div key={property.id} className="w-full flex flex-row gap-2 items-center">
                    ...
                    <div className="p-2">
                      <IconButton onClick={handlePropertyDelete.bind(null, property)}>
            ...
            selectedNode ? (
              <div className="flex flex-col gap-4">
                <div className="flex flex-col gap-2 overflow-y-auto max-h-96 pr-2">
                  ...
                  <div className="flex gap-2 items-top">
                    ...
                    <span className="text-gray-900 dark:text-white">{selectedNode.label}</span>
                  </div>

                  {Object.entries(selectedNode.properties).map(([key, value]) => (
                    <div key={key} className="flex gap-2 items-top">
                      <span className="text-gray-600 dark:text-gray-300">{key.charAt(0).toUpperCase() + key.slice(1)}:</span>
                      <span className="text-gray-900 dark:text-white">{typeof value === "object" ? JSON.stringify(value) : value as string}</span>
                    </div>
                  ))}
                </div>
              ...
              <span className="text-gray-900 dark:text-white">No node selected.</span>
            )
          )}
        </>
        {/* )} */}
